    ...
    <>
      ...
      */}
      <div style={{ display: activeSlice === "vle"            ? "contents" : "none" }}>
        <VLESlice           {...shared} onSlaChange={(ok) => onSlaChange("vle",            ok)} />
      </div>
      <div style={{ display: activeSlice === "student_portal" ? "contents" : "none" }}>
        <StudentPortalSlice {...shared} onSlaChange={(ok) => onSlaChange("student_portal", ok)} />
      </div>
      <div style={{ display: activeSlice === "admin"          ? "contents" : "none" }}>
        <AdminSlice         {...shared} onSlaChange={(ok) => onSlaChange("admin",          ok)} />
      </div>
      <div style={{ display: activeSlice === "iot"            ? "contents" : "none" }}>
        <IoTSlice           {...shared} onSlaChange={(ok) => onSlaChange("iot",            ok)} />
      </div>
      <div style={{ display: activeSlice === "general"        ? "contents" : "none" }}>
        <GeneralSlice       {...shared} onSlaChange={(ok) => onSlaChange("general",        ok)} />
      </div>
    </>
  );
}
