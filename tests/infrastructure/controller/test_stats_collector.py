...
# Unit tests for StatsCollector cache population, throughput computation,
# counter reset handling, and double-start guard.
...
def collector(config_files):
	return StatsCollector(topology_config=config_files, interval_sec=5)


@pytest.fixture
def multi_ap_collector(multi_ap_config):
	return StatsCollector(topology_config=multi_ap_config, interval_sec=5)
...
class TestStart:
	...
	def test_start_with_no_datapaths_does_not_raise(self, collector):
		with patch('infrastructure.controller.stats_collector.hub'):
			collector.start()
...
class TestGetStats:
	def test_returns_empty_dict_before_any_reply(self, collector):
...
class TestHandlePortStatsReply:
	...
	def test_populates_cache_with_correct_slice(self, collector):
		with patch('time.time', return_value=100.0):
			collector.handle_port_stats_reply('s2', [_make_stat(2, 0, 0)])
		with patch('time.time', return_value=101.0):
			collector.handle_port_stats_reply('s2', [_make_stat(2, 8_000_000, 4_000_000)])

		assert 'vle' in collector.get_stats()

	def test_throughput_computed_correctly(self, collector):
		with patch('time.time', return_value=100.0):
			collector.handle_port_stats_reply('s2', [_make_stat(2, 0, 0)])

		# 8_000_000 bytes in 1 second = 64_000_000 bps
		with patch('time.time', return_value=101.0):
			collector.handle_port_stats_reply('s2', [_make_stat(2, 8_000_000, 4_000_000)])

		stats = collector.get_stats()
		assert abs(stats['vle']['tx_throughput_bps'] - 64_000_000.0) < 1.0
		assert abs(stats['vle']['rx_throughput_bps'] - 32_000_000.0) < 1.0
	...
	def test_counter_reset_skips_interval(self, collector):
		...
		with patch('time.time', return_value=101.0):
			collector.handle_port_stats_reply('s2', [_make_stat(2, 100, 50)])
	...
	def test_counter_reset_next_interval_is_clean(self, collector):
		...
		# 900 bytes in 1 second = 7200 bps
		with patch('time.time', return_value=102.0):
			collector.handle_port_stats_reply('s2', [_make_stat(2, 1000, 500)])

		stats = collector.get_stats()
		assert abs(stats['vle']['tx_throughput_bps'] - 7200.0) < 1.0
	...
	def test_throughput_accumulated_across_ports_same_slice(self, multi_ap_collector):
		...
		# Each port: 8_000_000 bytes in 1s = 64_000_000 bps. Sum = 128_000_000 bps.
		with patch('time.time', return_value=101.0):
			multi_ap_collector.handle_port_stats_reply(
				's2',
				[
					_make_stat(2, 8_000_000, 0),
					_make_stat(3, 8_000_000, 0),
				],
			)
	...
	def test_packet_and_error_counts_present(self, collector):
		with patch('time.time', return_value=100.0):
			collector.handle_port_stats_reply(
				's2',
				[_make_stat(2, 0, 0, tx_packets=0, rx_packets=0, tx_errors=0, rx_errors=0)],
			)
		with patch('time.time', return_value=101.0):
			collector.handle_port_stats_reply(
				's2',
				[
					_make_stat(
						2, 1000, 500, tx_packets=10, rx_packets=5, tx_errors=1, rx_errors=0
					)
				],
			)

		stats = collector.get_stats()
		assert stats['vle']['tx_packets'] == 10
		assert stats['vle']['rx_packets'] == 5
		assert stats['vle']['tx_errors'] == 1
		assert stats['vle']['rx_errors'] == 0
	...
	def test_second_switch_reply_does_not_overwrite_first_slice(self, collector):
		with patch('time.time', return_value=100.0):
			collector.handle_port_stats_reply('s2', [_make_stat(2, 0, 0)])
			collector.handle_port_stats_reply('s3', [_make_stat(2, 0, 0)])
		...
		# s3 reply arrives with different data -- must not touch vle
		with patch('time.time', return_value=101.0):
			collector.handle_port_stats_reply('s3', [_make_stat(2, 1_000_000, 0)])

		stats = collector.get_stats()
		assert abs(stats['vle']['tx_throughput_bps'] - 64_000_000.0) < 1.0
		assert abs(stats['iot']['tx_throughput_bps'] - 8_000_000.0) < 1.0
