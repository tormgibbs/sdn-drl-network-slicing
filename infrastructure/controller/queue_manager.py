...
def _run(cmd: list[str], raise_on_error: bool = True) -> str:
...
def update_htb_queue(ap_name: str, max_rate_bps: int) -> None:
	port = _uplink_port(ap_name)
	qos_uuid = _run(['ovs-vsctl', 'get', 'Port', port, 'qos']).strip('[]"\n ')
	if not qos_uuid:
		logger.error('No QoS record found for port %s', port)
		return
	queue_uuid = _run(['ovs-vsctl', 'get', 'QoS', qos_uuid, 'queues:1']).strip('[]"\n ')
	if not queue_uuid:
		logger.error('No queue found in QoS record for port %s', port)
		return
	_run(
		['ovs-vsctl', 'set', 'Queue', queue_uuid, f'other-config:max-rate={max_rate_bps}']
	)
	logger.info('HTB queue updated: ap=%s max=%s bps', ap_name, max_rate_bps)


def destroy_htb_queues() -> None:
	_run(['ovs-vsctl', '--all', 'destroy', 'QoS'], raise_on_error=False)
	_run(['ovs-vsctl', '--all', 'destroy', 'Queue'], raise_on_error=False)
